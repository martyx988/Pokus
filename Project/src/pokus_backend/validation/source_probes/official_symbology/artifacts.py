...
def write_official_symbology_validation_artifact(
    *,
    output_path: Path,
...
def default_official_symbology_artifact_path() -> Path:
    return Path(__file__).with_name("artifacts").joinpath("m3_1_t68_live_probe_results.json")


def collect_official_symbology_records_for_run(session: Any, *, run_key: str) -> list[Any]:
